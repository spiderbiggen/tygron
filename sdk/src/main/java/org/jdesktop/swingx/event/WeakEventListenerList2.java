/*
 * $Id: WeakEventListenerList.java 3190 2009-01-20 17:47:52Z kschaefe $ Copyright 2006 Sun Microsystems, Inc., 4150 Network Circle, Santa
 * Clara, California 95054, U.S.A. All rights reserved. This library is free software; you can redistribute it and/or modify it under the
 * terms of the GNU Lesser General Public License as published by the Free Software Foundation; either version 2.1 of the License, or (at
 * your option) any later version. This library is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even
...
/**
 * A class that holds a list of EventListeners. A single instance can be used to hold all listeners (of all types) for the instance using
 * the list. It is the responsibility of the class using the EventListenerList to provide type-safe API (preferably conforming to the
 * JavaBeans spec) and methods which dispatch event notification methods to appropriate Event Listeners on the list.
 * 
 * The main benefit that this class provides is that it releases garbage collected listeners (internally uses weak references).
 * <p>
 * 
 * PENDING: serialization support
 * 
 *
 * Usage example: Say one is defining a class that sends out FooEvents, and one wants to allow users of the class to register FooListeners
 * and receive notification when FooEvents occur. The following should be added to the class definition:
 * 
 * <pre>
 ...
 * <p>
 * <strong>Warning:</strong> Serialized objects of this class will not be compatible with future Swing releases. The current serialization
 * support is appropriate for short term storage or RMI between applications running the same version of Swing. As of 1.4, support for long
 * term storage of all JavaBeans<sup><font size="-2">TM</font></sup> has been added to the <code>java.beans</code> package. Please see
 ...
 */
public class WeakEventListenerList2 implements Serializable {

    /**
     * 
     */
    private static final long serialVersionUID = 2363513010704292806L;
    ...
    /**
     * Passes back the event listener list as an array of ListenerType-listener pairs. As a side-effect, cleans out any garbage collected
